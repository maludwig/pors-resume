...
//Rotates a point by rad degrees around another point
Point.prototype.rotate = function(rad,pt2) {
	var v = new Vector(pt2,this);
	return pt2.add(v.rotate(rad));
};
...
function Box(x1,y1,x2,y2) {
	...
	this.midpoint = new Point(this.p1.midpoint(this.p2));
	this.left = this.x1;
	this.top = this.y1;
	this.right = this.x2;
	this.bottom = this.y2;
...
//If there are no inputs, it is a Vector with zero magnitude.
function Vector(o,t) {
...
//Averages the input Vector and the current Vector, returns the result as a new Vector
Vector.prototype.average = function(v) {
	return new Vector((this.dx + v.dx)/2, (this.dy + v.dy)/2);
};
